...
public class AdminController {
    ...
    public Result getSdModelPages(@RequestParam(defaultValue = "1") final int pageNum, final String prompt) {
        log.info("pageNum: {}, prompt: {}", pageNum, prompt);
    ...
    public Result getServerConfig() {
        log.info("获取服务器配置");
        Result data = Result.data(serverService.getDisposition());
        log.info("data:{}", data);
        return data;
    }
     ...
     * @return the bot configuration
     */
    @GetMapping(value = "/server/get/terminal", name = "获取终端", produces = MediaType.APPLICATION_JSON_VALUE)
     ...
     * @return the bot configuration
     */
    @GetMapping(value = "/product/get/page", name = "分页获取用户信息", produces = MediaType.APPLICATION_JSON_VALUE)
    ...
    public Result getDrawingPage(@RequestParam(defaultValue = "1") final int pageNum) {
        log.info("获取绘图列表分页 pageNum:{}", pageNum);
