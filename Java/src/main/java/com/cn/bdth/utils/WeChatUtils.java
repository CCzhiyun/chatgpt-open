        ...
        try {
            ...
            final byte[] block = WEB_CLIENT.post()
                    ...
                    .body(BodyInserters.fromValue(
                            new WeChaQrCodeModel()
                            .setScene(secene)
                            .setEnv_version(controlCommon.getControl().getWechatAppEnv())
                    ))
                    .retrieve().bodyToMono(byte[].class).block();

            String picData = "data:image/png;base64," +Base64.getEncoder().encodeToString(block);
            log.info("获取小程序二维码成功, 二维码:{}", picData);
            return picData;
        } catch (Exception e) {
